...
مقایسه‌ی چهار استراتژی تصمیم‌گیری (Greedy، Kubernetes-HPA، VOILA، PPO-DRL) هم در قالب شبیه‌سازی discrete-event و هم در اجرای واقعی روی Kubernetes. این سند شامل تمام فرمول‌ها، ثابت‌ها، ساختار داده و منطق تصمیم‌گیری لازم برای پیاده‌سازی کامل پروژه است.
...
5. [پیکربندی کامل ثابت‌های سیستم](#۵-پیکربندی-کامل-ثابت‌های-سیستم)
...
22. [ترتیب منطقی ساخت پروژه از صفر](#۲۲-ترتیب-منطقی-ساخت-پروژه-از-صفر)
...
هدف این است که هم‌زمان تأخیر پاسخ‌دهی پایین بماند، سررسیدها (deadline) نقض نشوند، و مصرف انرژی هم کنترل‌شده باشد. چهار استراتژی تصمیم‌گیری روی داده‌ی واقعی ترافیک BTS شانگهای، با معیارهای یکسان، پیاده‌سازی و با هم مقایسه می‌شوند:
...
| **HPA** | معادل Kubernetes Horizontal Pod Autoscaler: عمداً مکان‌ناآگاه است. فقط بر اساس نسبت اشغال صف نسبت به یک هدف ثابت (۷۰٪) تعداد replica مطلوب را حساب می‌کند و جای‌گذاری را فقط از روی ظرفیت آزاد انتخاب می‌کند. |
...
| **PPO-DRL** | عامل یادگیری تقویتی (Proximal Policy Optimization با MaskablePPO) که هر سه نوع تصمیم (اسکیل، provisioning، جای‌گذاری) را هم‌زمان از یک بردار حالت یاد می‌گیرد. آموزش با warm-start از دموی Greedy (Behavior Cloning) شروع و سپس با RL روی یک پاداش وزن‌دار fine-tune می‌شود. |
...
### ۲.۴ زمان اجرا (heterogeneity-aware)
...
### ۲.۵ تبدیل واحد MIPS خام به مقدار مؤثر
...
**بررسی شدنی‌بودن SLA** (`is_sla_feasible`): قبل از این‌که یک سرور برای میزبانی یک سرویس پذیرفته شود، بررسی می‌شود که آیا با در نظر گرفتن بدترین حالت زمان اجرا و تأخیر شبکه، امکان برآورده‌شدن deadline آن سرویس وجود دارد یا نه:

```python
def is_sla_feasible(service_id, server_lat, server_long, host_mips_per_core,
                     bts_lat=None, bts_long=None):
    svc = SERVICES_INFO[service_id]
    exec_time = compute_exec_time_sec(service_id, host_mips_per_core)
    if bts_lat is not None and bts_long is not None:
        distance_km = haversine_km(bts_lat, bts_long, server_lat, server_long)
    else:
        # مسیر محافظه‌کارانه: بدترین فاصله‌ی ممکن در کل محدوده‌ی جغرافیایی
        distance_km = haversine_km(LAT_MIN, LON_MIN, LAT_MAX, LON_MAX)
    delay_ms = network_delay_ms(distance_km, BASE_LATENCY_MS, K_MS_PER_KM) + DISPATCH_OVERHEAD_MS
    rtt_sec = 2 * delay_ms / 1000.0
    worst_case_response = rtt_sec + exec_time
    return worst_case_response <= svc["deadline"]
```
...
- اگر در استخر نزدیک هیچ گزینه‌ای پذیرفته نشد، جست‌وجو با ترتیب فاصله روی بقیه‌ی کاندیداها ادامه می‌یابد.
...
```python
def select_replica_latency_aware(request, candidate_replicas, servers, now, occupancy_fn):
    best_r, best_latency = None, float("inf")
    scored = []
    for r in candidate_replicas:
        occ = occupancy_fn(r)
        if occ >= r.queue_len:
        ...
        scored.append((est_total_latency, r))
    scored.sort(key=lambda pair: pair[0])
    for _, r in scored:
        return r   # اولین کاندیدای مرتب‌شده که ظرفیت خالی داشت
    return None
```
...
الگوریتم پوشش حریصانه (greedy set cover) برای انتخاب مجموعه‌ی سرورهایی که در ابتدا روشن می‌شوند:
...
پیاده‌سازی pseudocode با pulp:

```python
import pulp

def ilp_initial_placement(servers, demand_points, w_count=0.3, w_energy=0.3, w_distance=0.4):
    prob = pulp.LpProblem("initial_placement", pulp.LpMinimize)
    x = {sid: pulp.LpVariable(f"x_{sid}", cat="Binary") for sid in servers}          # سرور روشن یا نه
    y = {(sid, k): pulp.LpVariable(f"y_{sid}_{k}", cat="Binary")
         for sid in servers for k in range(len(demand_points))}                        # انتصاب نقطه‌ی تقاضا k به سرور sid

    total_p_idle = sum(SERVER_PROFILES[s.profile]["p_idle"] for s in servers.values())
    max_distance = haversine_km(LAT_MIN, LON_MIN, LAT_MAX, LON_MAX)

    prob += (
        w_count * (pulp.lpSum(x.values()) / len(servers))
        + w_energy * (pulp.lpSum(x[sid] * SERVER_PROFILES[servers[sid].profile]["p_idle"]
                                  for sid in servers) / total_p_idle)
        + w_distance * pulp.lpSum(
            y[(sid, k)] * (haversine_km(*demand_points[k], servers[sid].lat, servers[sid].long) / max_distance)
            for sid in servers for k in range(len(demand_points))
          ) / max(len(demand_points), 1)
    )

    for k in range(len(demand_points)):
        prob += pulp.lpSum(y[(sid, k)] for sid in servers) <= 1     # هر نقطه حداکثر به یک سرور
    for sid in servers:
        for k in range(len(demand_points)):
            prob += y[(sid, k)] <= x[sid]                            # فقط سرور فعال می‌تواند انتصاب بگیرد

    total_demand = sum(s["resource_mips"] for s in SERVICES_INFO.values())
    prob += pulp.lpSum(x[sid] * servers[sid].capacity for sid in servers) >= total_demand

    prob.solve(pulp.PULP_CBC_CMD(msg=False))
    if pulp.LpStatus[prob.status] != "Optimal":
        return None   # fallback به پوشش حریصانه‌ی بخش ۹.۱
    return [sid for sid in servers if pulp.value(x[sid]) > 0.5]
```

قیود: هر نقطه‌ی تقاضای پوشش‌پذیر حداکثر به یک سرور فعال منصوب می‌شود؛ انتصاب فقط به سرور فعال‌شده مجاز است؛ مجموع ظرفیت سرورهای انتخاب‌شده باید حداقل به‌اندازه‌ی مجموع نیاز ۱۵ سرویس باشد. نقاط تقاضا از کل تایم‌لاین سه‌روزه‌ی آموزش استخراج می‌شوند (نه فقط پنجره‌ی ابتدایی). اگر solver جواب پیدا نکرد یا کتابخانه نصب نبود، به همان استراتژی پوشش حریصانه‌ی بخش ۹.۱ برمی‌گردد.
...
def any_active_server_sustained_underloaded(now):
    for s in servers.values():
        if s.state != ACTIVE: continue
        if s.instantaneous_utilization(now) < UTIL_SCALE_DOWN_THRESHOLD:
            if s.underload_since is None: s.underload_since = now
            if now - s.underload_since >= SUSTAIN_LOW_SEC: return True
        else:
            s.underload_since = None
    return False

def any_service_capacity_starved(snapshot):
    return any(svc["rejection_rate"] > 0 for svc in snapshot["services"].values())
```

این سیگنال‌ها برای گزارش و ممیزی محاسبه می‌شوند؛ گیت‌های واقعی که تصمیم‌ها را عملاً مسدود می‌کنند، cooldown (۶۰ ثانیه بعد از هر تغییر حالت روی همان سرور) و حداقل مدت فعال بودن (`MIN_ACTIVE_DURATION_SEC=300`) هستند.
...
`start_server_boot` سرور را به حالت `BOOTING` می‌برد و یک رویداد «پایان بوت» را `BOOT_DELAY_SEC` ثانیه بعد زمان‌بندی می‌کند؛ وقتی آن رویداد اجرا شد، سرور به `ACTIVE` می‌رود، `last_transition_time` به‌روزرسانی می‌شود، و `E_BOOT_SERVER_J` ژول انرژی بوت ثبت می‌شود.
...
منطق `trigger_emergency_boot`: نزدیک‌ترین سرور خاموش که مجموع ظرفیت آزادش برای میزبانی همه‌ی سرویس‌های `unmigrated` کافی باشد پیدا و بلافاصله بوت می‌شود (خارج از چرخه‌ی معمول تصمیم‌گیری، چون این یک وضعیت اضطراری است)؛ اگر چنین سروری نبود، سرور مبدأ به‌ناچار فعال باقی می‌ماند تا در دور بعدی دوباره تلاش شود.
...
```python
def on_server_drain_done(server_id):
    s = servers[server_id]
    if all(r.state == TERMINATED for r in s.hosted_replicas.values()):
        s.state = OFF; s.last_transition_time = now
        s.hosted_replicas = {}
    else:
        schedule(now + SERVER_DRAIN_GRACE_SEC, SERVER_DRAIN_DONE, server_id)   # به تعویق می‌افتد
```

وقتی رپلیکای مقصد یک مهاجرت به حالت `READY` می‌رسد، همان لحظه رپلیکای متناظر روی سرور مبدأ (`pending_migrations`) وارد `start_replica_drain` می‌شود.
...
«نقض نزدیکی جغرافیایی» یعنی نسبت درخواست‌های اخیر آن سرویس که تأخیر شبکه‌شان از `PROXIMITY_L0_MS=7.0` میلی‌ثانیه بیشتر بوده، مثبت است.

**PPO:** تصمیم از خروجی مدل آموزش‌دیده می‌آید (بخش ۱۴).

### انتخاب رپلیکای قربانی هنگام کاهش مقیاس

```python
def select_scale_down_victim(ready_replicas, now):
    mature = [r for r in ready_replicas if (now - r.created_at) >= MIN_REPLICA_AGE_BEFORE_SCALE_DOWN_SEC]
    if not mature: return None
    return min(mature, key=lambda r: r.queue_occupancy(now))   # پیش‌فرض: کمترین اشغال
```

VOILA و Greedy از همین پیش‌فرض استفاده می‌کنند؛ HPA هم چون مکان‌ناآگاه است همین قاعده را به کار می‌برد.
...
def demand_centroid(service_id):
    buf = recent_arrival_positions[service_id]   # دِک با حداکثر ۳۰ عضو، جدیدترین آخر
    return medoid(list(buf))
```

هر بار درخواست جدیدی برای یک سرویس می‌رسد، موقعیت آن به انتهای بافر همان سرویس اضافه می‌شود؛ اگر بافر پر باشد، قدیمی‌ترین موقعیت حذف می‌شود (FIFO با ظرفیت ۳۰).
...
```python
def compute_load_balance_cv(now):
    utils = [s.instantaneous_utilization(now) for s in servers.values() if s.state == ACTIVE]
    if len(utils) < 2 or mean(utils) == 0: return 0.0
    return std(utils) / mean(utils)
```
...
class EventType(Enum):
    REQUEST_ARRIVAL = auto(); REQUEST_ROUTED = auto(); DECISION_TICK = auto()
    SERVER_BOOT_DONE = auto(); SERVER_DRAIN_DONE = auto()
    REPLICA_READY = auto(); REPLICA_REMOVED = auto(); ENERGY_SYNC = auto()

def run_simulation():
    load_all_arrival_events_into_heap()
    initial_servers = algorithm.initial_placement(servers, active_bts)
    for sid in initial_servers: start_server_boot(sid)
    schedule(0.0, DECISION_TICK)

    end_marker = last_arrival_time + SERVER_DRAIN_GRACE_SEC + GRACEFUL_TERMINATION_DELAY_SEC
    while event_queue and event_queue[0].time <= end_marker:
        event = heappop(event_queue)
        integrate_energy_up_to(event.time)
        dispatch(event)
...
4. قبل از پردازش هر رویداد، انرژی مصرفی همه‌ی سرورها تا همان لحظه integrate می‌شود (انتگرال‌گیری از توان لحظه‌ای نسبت به فاصله‌ی زمانی از آخرین همگام‌سازی).
...
```python
def on_decision_tick(now):
    snapshot = build_metrics_snapshot(now)
    for svc in ACTIVE_SERVICES:
        decision = algorithm.scale_decision(svc, snapshot)
        apply_scale_decision(svc, decision, snapshot, now)
    provision_action = algorithm.provision_decision(servers, snapshot, now)
    apply_provisioning(provision_action, snapshot, now)
    schedule(now + DECISION_INTERVAL_SEC, DECISION_TICK)
```
...
برای هر سرور (به ترتیب شماره‌ی سرور):
  یک بردار one-hot از حالت (خاموش/بوت/فعال/تخلیه)     -> ۴ بعد
  بهره‌وری لحظه‌ای                                      -> ۱ بعد
  نسبت تعداد رپلیکاهای میزبانی‌شده به کل سرویس‌ها       -> ۱ بعد
  جمع: ۶ بعد در هر سرور × ۱۰ سرور = ۶۰ بعد
...
برای هر سرویس (به ترتیب شماره‌ی سرویس):
  ...
  جمع: ۶ بعد در هر سرویس × ۱۵ سرویس = ۹۰ بعد
...
پیاده‌سازی ساخت بردار:

```python
def build_state_vector(snapshot, servers):
    vec = []
    for sid in sorted_server_ids:
        s = servers[sid]
        one_hot = [1.0 if s.state == st else 0.0 for st in (OFF, BOOTING, ACTIVE, DRAINING)]
        util = s.instantaneous_utilization(now)
        replica_ratio = len(s.hosted_replicas) / N_SERVICES
        vec.extend(one_hot + [util, replica_ratio])

    for sid in sorted_service_ids:
        sv = snapshot["services"][sid]
        replica_ratio = sv["n_replicas"] / N_SERVERS
        occ_ratio = min(sv["avg_queue_occupancy_ratio"], 1.0)
        dv_rate = sv["deadline_violation_rate"]
        arrival_rate = min(sv["recent_arrival_rate"] / NORM_ARRIVAL_RATE, 2.0)
        rejection_rate = sv["rejection_rate"]
        proximity_violation_rate = sv["proximity_violation_rate"]
        vec.extend([replica_ratio, occ_ratio, dv_rate, arrival_rate, rejection_rate, proximity_violation_rate])

    g = snapshot["global"]
    vec.append(min(g["avg_response_time_recent"] / NORM_RESPONSE_TIME_SEC, 2.0))
    vec.append(min(g["energy_recent_joule"] / NORM_ENERGY_JOULE, 2.0))
...
اکشن یک بردار چند-گسسته است: ۱۵ بعد اول (به ازای هر سرویس: بدون تغییر / افزایش / کاهش مقیاس) و ۱۰ بعد بعدی (به ازای هر سرور: بدون تغییر / روشن‌کردن / خاموش‌کردن).

```python
SCALE_MAP = {0: NO_CHANGE, 1: SCALE_UP, 2: SCALE_DOWN}
PROVISION_MAP = {0: NO_CHANGE, 1: TURN_ON, 2: TURN_OFF}
action_space = MultiDiscrete([3] * 15 + [3] * 10)
```

چون در هر تیک فقط یک عمل provisioning واقعی به موتور پاس داده می‌شود، از میان همه‌ی گزینه‌های غیر-«بدون تغییر» بخش سرور، اولویت با روشن‌کردن است؛ اگر چند سرور هم‌زمان انتخاب شده باشند، سروری با کمترین شماره برنده می‌شود.
...
class EdgeRLEnv(gym.Env):
    def __init__(self, data_window, seed):
        self.observation_space = spaces.Box(low=0.0, high=2.0, shape=(152,), dtype=np.float32)
        self.action_space = spaces.MultiDiscrete([3]*15 + [3]*10)
        self.engine = SimulationEngine(data_window, algorithm=None, seed=seed)

    def reset(self, seed=None, options=None):
        self.engine.reset(seed=seed)
        self.engine.run_until_first_decision_tick()
        obs = build_state_vector(self.engine.snapshot(), self.engine.servers)
        return obs, {}

    def step(self, action):
        scale_actions = {sid: SCALE_MAP[action[i]] for i, sid in enumerate(sorted_service_ids)}
        provision_action = self._resolve_provision_action(action[15:])
        n_applied = self.engine.apply_external_decision(scale_actions, provision_action)
        self.engine.run_until_next_decision_tick()
        snapshot = self.engine.snapshot()
        obs = build_state_vector(snapshot, self.engine.servers)
        reward = compute_reward(snapshot, n_applied)
        terminated = self.engine.is_finished()
        action_masks = compute_action_masks(self.engine, snapshot)
        return obs, reward, terminated, False, {"action_masks": action_masks}

    def action_masks(self):
        return compute_action_masks(self.engine, self.engine.snapshot())
```

در هر گام، اکشن مدل به یک تصمیم اسکیل برای هر سرویس و یک تصمیم provisioning ترکیبی تبدیل می‌شود، به موتور شبیه‌سازی پاس داده می‌شود، و پاداش از روی دلتای تعداد اکشن‌های واقعاً اعمال‌شده محاسبه می‌شود.
...
model = MaskablePPO(
    ...
    n_steps=2048, batch_size=256, gamma=0.99, learning_rate=3e-4, ent_coef=0.01,
    verbose=1, seed=SEED,
)
# اگر وزن‌های BC از پیش آماده باشند، همین‌جا روی model.policy لود می‌شوند.
...
async def route_request(req: RouteRequest):
    ready_servers = redis.smembers(f"edge:service:{req.service_id}:ready_replicas")
    candidates = [build_candidate(sid, req.service_id) for sid in ready_servers]
    chosen = select_replica(req, candidates, servers_cache, now=time.time())
    if chosen is None:
        return {"status": "rejected"}
    reservation_id = str(uuid4())
    expiry = time.time() + SERVICES_INFO[req.service_id]["deadline"] + 5.0
    redis.hset(f"edge:reservations:{req.service_id}:{chosen.server_id}", reservation_id, expiry)
    redis.incr(f"edge:replica:{req.service_id}:{chosen.server_id}:queue")
    pod_ip = redis.get(f"edge:replica:{req.service_id}:{chosen.server_id}:pod_ip")
    return {"status": "routed", "server_id": chosen.server_id, "pod_ip": pod_ip,
            "reservation_id": reservation_id}

@app.post("/report_completion")
async def report_completion(rep: CompletionReport):
    redis.lpush("edge:metrics:completions", rep.json())
    redis.decr(f"edge:replica:{rep.service_id}:{rep.server_id}:queue")
```

### ۱۵.۴ Provisioning و اسکیل در حالت بلادرنگ
...
async def decision_loop():
    while running:
        ...
        for svc in ACTIVE_SERVICES:
            decision = algorithm.scale_decision(svc, snapshot)
            await apply_scale_decision_async(svc, decision, snapshot)
        provision_action = algorithm.provision_decision(servers_cache, snapshot, time.time())
        await apply_provisioning_async(provision_action, snapshot)
        await asyncio.sleep(DECISION_INTERVAL_SEC)

async def completion_drain_loop():
    while running:
        msg = redis.rpop("edge:metrics:completions")
        if msg: process_completion(msg)
        await asyncio.sleep(0.2)

async def utilization_energy_sample_loop():
    while running:
        sample_all_servers_utilization_and_energy()
...
### ۱۵.۵ بازیابی اطلاعات جغرافیایی برای متریک نهایی
...
def create_deployment(service_id, server_id):
    spec = client.V1Deployment(
        metadata=client.V1ObjectMeta(name=f"svc-{service_id}-srv-{server_id}", namespace=NAMESPACE),
        spec=client.V1DeploymentSpec(
            replicas=1,
            selector=client.V1LabelSelector(match_labels={"service": str(service_id), "server": str(server_id)}),
            template=client.V1PodTemplateSpec(
                metadata=client.V1ObjectMeta(labels={"service": str(service_id), "server": str(server_id)}),
                spec=client.V1PodSpec(
                    host_network=True,
                    node_selector={NODE_LABEL_KEY: str(server_id)},
                    containers=[client.V1Container(
                        name="worker", image="edge-rl-worker:latest",
                        env=[client.V1EnvVar(name="SERVICE_ID", value=str(service_id)),
                             client.V1EnvVar(name="SERVICE_PORT", value=str(worker_port(service_id))),
                             client.V1EnvVar(name="QUEUE_LEN", value=str(SERVICES_INFO[service_id]["queue_len"]))],
                        resources=client.V1ResourceRequirements(
                            requests={"cpu": f"{resource_mips_to_millicpu(SERVICES_INFO[service_id]['resource_mips'])}m",
                                      "memory": SERVICES_INFO[service_id]["memory"]}),
                        readiness_probe=client.V1Probe(
                            http_get=client.V1HTTPGetAction(path="/health", port=worker_port(service_id))),
                    )],
                ),
            ),
        ),
    )
    return retry_with_backoff(lambda: apps_api.create_namespaced_deployment(NAMESPACE, spec))

def delete_deployment(service_id, server_id):
    name = f"svc-{service_id}-srv-{server_id}"
    return retry_with_backoff(lambda: apps_api.delete_namespaced_deployment(name, NAMESPACE))

def is_deployment_ready(service_id, server_id):
    name = f"svc-{service_id}-srv-{server_id}"
...
def get_pod_ip(service_id, server_id):
    pods = core_api.list_namespaced_pod(
        NAMESPACE, label_selector=f"service={service_id},server={server_id}")
    return pods.items[0].status.pod_ip if pods.items else None

def cordon_node(server_id):   # معادل خاموش‌کردن سرور
    node_name = node_name_for_server(server_id)
    body = {"spec": {"unschedulable": True}}
    return retry_with_backoff(lambda: core_api.patch_node(node_name, body))
...
app = FastAPI()
service_id = int(os.environ["SERVICE_ID"])
svc = SERVICES_INFO[service_id]
semaphore = asyncio.Semaphore(1)   # فقط یک درخواست هم‌زمان
...
async def process(req: ProcessRequest):
    async with semaphore:
        exec_time = compute_exec_time_sec(service_id, HOST_MIPS_PER_CORE)
        if req.is_cold_start:
            exec_time += compute_cold_start_penalty_sec(service_id, HOST_MIPS_PER_CORE)
        await asyncio.sleep(exec_time)
        report_completion_to_dispatcher(req)
...
هر رویداد مهم سیستم (ورود درخواست، تکمیل، رد‌شدن، تغییر حالت سرور یا رپلیکا، تصمیم اسکیل/provisioning، مهاجرت، روشن‌کردن اضطراری، جاروب رزرو و غیره) به‌صورت یک رکورد JSON مستقل در یک فایل خط‌به‌خط (JSONL) ثبت می‌شود:
...
هر رکورد شامل نوع رویداد، نام الگوریتم، زمان شبیه‌سازی، زمان واقعی، و فیلدهای اختصاصی همان رویداد است. خروجی هر اجرا شامل یک فایل لاگ رویدادها و یک فایل خلاصه‌ی نتایج نهایی است.

---

## ۱۷. بارگذاری داده

داده‌ی خام شامل چهار فایل CSV است (سه روز برای آموزش، یک روز مستقل برای تست) با ستون‌های شناسه، شناسه‌ی BTS، عرض/طول جغرافیایی، شناسه‌ی سرویس و زمان شروع.

```python
def load_and_preprocess(files, data_dir):
    frames = []
    for day_index, fname in enumerate(files):
        df = pd.read_csv(os.path.join(data_dir, fname))
        df = df[(df.lat >= LAT_MIN) & (df.lat <= LAT_MAX) &
                (df.long >= LON_MIN) & (df.long <= LON_MAX)]
        df = df[df.service_id.isin(ACTIVE_SERVICES)]
        df["global_time"] = day_index * SECONDS_PER_DAY + df["start_time_sec"]
        frames.append(df)
    full = pd.concat(frames, ignore_index=True).sort_values("global_time")
    return full.reset_index(drop=True)
```

هنگام بارگذاری، رکوردهای خارج از محدوده‌ی جغرافیایی معتبر و سرویس‌های غیرفعال حذف می‌شوند، و یک ستون زمان سراسری (بر اساس شماره‌ی روز ضرب در ۸۶۴۰۰ ثانیه به‌علاوه‌ی زمان درون‌روزی) ساخته می‌شود تا چند روز به یک تایم‌لاین پیوسته تبدیل شوند.

---

## ۱۸. رابط مشترک الگوریتم‌ها

هر الگوریتم تصمیم‌گیری از یک کلاس پایه‌ی مشترک ارث‌بری می‌کند:
...
class AlgorithmBase:
    ...
    def initial_placement(self, servers, active_bts):
        return shared_initial_placement(self, servers, active_bts)   # بخش ۹

    def select_replica(self, request, candidate_replicas, servers, now, **kwargs):
        return shared_select_replica(request, candidate_replicas, servers, now, **kwargs)   # بخش ۸

    def select_scale_down_victim(self, ready_replicas, now):
        return shared_select_scale_down_victim(ready_replicas, now)   # پیش‌فرض: کمترین اشغال

    def scale_decision(self, service_id, metrics_snapshot):
        raise NotImplementedError   # اختصاصی هر الگوریتم، بخش ۱۱

    def provision_decision(self, servers, metrics_snapshot, now):
        raise NotImplementedError   # اختصاصی هر الگوریتم، بخش ۱۰

    def select_placement_server(self, service_id, active_servers, cpu_demand):
        raise NotImplementedError   # اختصاصی هر الگوریتم، بخش ۱۱

    def migration_decision(self, draining_server, servers):
        raise NotImplementedError   # بخش ۱۰
```
...
متغیرهای محیطی قابل تنظیم: مسیر پوشه‌ی داده (`EOTCH_DATA_DIR`)، seed اجرا (`EOTCH_SEED`، پیش‌فرض ۴۲)، و ضریب نرمال‌سازی نرخ رد‌شدن در پاداش PPO (`EOTCH_NORM_REJECTED_PER_TICK`، پیش‌فرض ۲.۰).
...
python analyze_decision_quality.py outputs/seed42/ppo_events.jsonl

python python diagnose_violations_by_service.py outputs/seed42/ppo_events.jsonl

python analyze_scaleup_by_service.py outputs/seed42/ppo_events.jsonl
...
## ۲.۱ (جدول تکمیلی) مشخصات ۱۰ سرور

| شماره سرور | شناسه BTS | عرض جغرافیایی | طول جغرافیایی | ظرفیت (MIPS) | پروفایل |
|---|---|---|---|---|---|
| 1 | 1498 | 31.37 | 121.25 | 20160 | edge_small |
| 2 | 777 | 31.31 | 121.51 | 20160 | edge_small |
| 3 | 530 | 31.10 | 121.18 | 33120 | medium |
| 4 | 121 | 31.25 | 121.37 | 33120 | medium |
| 5 | 292 | 31.10 | 121.36 | 33120 | medium |
| 6 | 1344 | 31.04 | 121.74 | 33120 | medium |
| 7 | 182 | 31.17 | 121.57 | 40320 | large |
| 8 | 505 | 31.15 | 121.41 | 40320 | large |
| 9 | 419 | 31.20 | 121.43 | 40320 | large |
| 10 | 609 | 31.16 | 121.49 | 40320 | large |

## ۲.۲ (جدول تکمیلی) مشخصات ۱۵ سرویس

| شماره سرویس | مصرف CPU (MIPS) | طول کار (میلیون دستور) | ظرفیت صف | سررسید (ثانیه) | حافظه |
|---|---|---|---|---|---|
| 1 | 4800 | 55 | 1 | 0.030 | 32Mi |
| 2 | 4900 | 110 | 2 | 0.050 | 48Mi |
| 3 | 5000 | 140 | 2 | 0.060 | 48Mi |
| 4 | 5100 | 190 | 3 | 0.075 | 56Mi |
| 5 | 5200 | 260 | 3 | 0.100 | 64Mi |
| 6 | 5300 | 310 | 3 | 0.100 | 72Mi |
| 7 | 5400 | 420 | 4 | 0.150 | 96Mi |
| 8 | 5400 | 570 | 5 | 0.200 | 128Mi |
| 9 | 5500 | 880 | 6 | 0.300 | 160Mi |
| 10 | 5600 | 1050 | 6 | 0.300 | 176Mi |
| 11 | 5400 | 3500 | 8 | 1.0 | 192Mi |
| 12 | 5400 | 7000 | 10 | 2.0 | 256Mi |
| 13 | 5200 | 10000 | 12 | 3.0 | 320Mi |
| 14 | 5200 | 50500 | 15 | 15.0 | 512Mi |
| 15 | 5800 | 151000 | 20 | 40.0 | 768Mi |

---

**جمع‌بندی:** این سند تمام منابع، فرمول‌ها، ثابت‌ها، ساختار داده، و منطق تصمیم‌گیری چهار الگوریتم (Greedy، HPA، VOILA، PPO-DRL) را — هم در مسیر شبیه‌سازی discrete-event و هم در مسیر اجرای واقعی روی Kubernetes — با جزئیات کامل کد و فرمول پوشش می‌دهد، به‌گونه‌ای که بدون نیاز به منبع دیگری قابل پیاده‌سازی است.
